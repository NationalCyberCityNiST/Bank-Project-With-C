...
unsigned int char_to_integer_fun(char char_array[]);
...
void money_transaction(int transmit , int receiver , unsigned int amount){
    ...
    printf("\n\n After copy two char array %s\n",_month[0].str_month);
        ...
        transacitonRecord(transmit,receiver,'t',amount);
        transacitonRecord(transmit,receiver,'r',amount);
...
void transacitonRecord(int userDBIndex,int uFound,char who,unsigned int amount){
    ...
    if(who == 't') {
        ...
        for(int i=endpoint;i<toendpoint;i++){
            db[uFound].tr[space_array[uFound]-19].note[i]=db[userDBIndex].name[reIndex];
            reIndex++;
        }
...
unsigned int char_to_integer_fun(char char_array[]){
    ...
    if(fptr2 == NULL){
        ...
        fscanf(fptr,"%u",&char_to_int_data);
    }
...
int total_money_for_same_days(int transmit,int last_day){
    ...
    int to_get_all_records=space_array[transmit]-19;
